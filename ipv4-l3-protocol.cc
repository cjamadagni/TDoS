...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  //NS_LOG_INFO("AT NODE " << m_node->GetId ()<<":");
  // Forwarding
  ...
  //Setting the malicious node as node1D = 2
  if(m_node->GetId()==2) {
  NS_LOG_INFO("AT MALICIOUS NODE (Node 2): ");  
  //int randomno = (rand()%5)+1;
  //int randomno = rand()%7;
  packet_Count++;  
  // Deciding whether to attack the packet
  ...
  if((packet_Count%7==0 || packet_Count%7==1 || packet_Count%7==2 || packet_Count%7==3) && ipHeader.GetProtocol () != Icmpv4L4Protocol::PROT_NUMBER && 
          ipHeader.GetDestination ().IsBroadcast () == false &&
          ipHeader.GetDestination ().IsMulticast () == false && ipHeader.GetTtl () >= 3)
  {

    int randomno2 = (rand()%2)+1;
    ipHeader.SetTtl(randomno2);
    std::cout<<"TTL changed maliciously to "<<randomno2<<std::endl;
  }
  else
    ipHeader.SetTtl (ipHeader.GetTtl () - 1);
  //NS_LOG_INFO("TTL="<<(int)ipHeader.GetTtl ());
  if (ipHeader.GetTtl () <= 0)
    {
      ...
      if (ipHeader.GetProtocol () != Icmpv4L4Protocol::PROT_NUMBER && 
          ...
          ipHeader.GetDestination ().IsMulticast () == false)
        {
          ...
          std::cout<<"ICMP sent: "<<icmp<<std::endl;    
        }         
      NS_LOG_WARN ("TTL exceeded.  Drop.");
